...
equation
  connect(heatStorageAndTransmission.external_port_l, port_a) annotation (Line(
        points={{-10,-60},{-80,-60},{-80,0},{-100,0}}, color={191,0,0}));
  connect(heatStorageAndTransmission.external_port_r, port_b) annotation (Line(
        points={{10,-60},{80,-60},{80,0},{100,0}}, color={191,0,0}));
  ...
  connect(abnormalHeatGenerationWithStatus.tr_in_progress,
    ...
    annotation (Line(points={{0,33},{0,48}}, color={255,0,255}));
  connect(abnormalHeatGenerationWithStatus.thermal_port, CellTemperature)
    annotation (Line(points={{0,12},{0,-30}}, color={191,0,0}));
  connect(heatStorageAndTransmission.internal_port, CellTemperature)
    annotation (Line(points={{0,-50},{0,-30}}, color={191,0,0}));
